...
class StockDataGenerator:
    ...
    def __init__(self):
        # Базові параметри для основних тікерів
        self.ticker_params = {
    ...
    def gen_stock_data(self, ticker, days_back=90):
        # Встановлюємо seed на основі тікера для стабільності результатів
        np.random.seed(sum(ord(c) for c in ticker))
        
        # Отримуємо параметри для тікера або використовуємо стандартні
        params = self.ticker_params.get(ticker, {'price': 100.0, 'volatility': 0.02, 'trend': 0.0004})
        start_price = params['price']
        volatility = params['volatility']
        trend = params['trend']
        
        # Генеруємо дати (тільки робочі дні)
        ...
        # Генеруємо ціни закриття
        n_days = len(dates)
        close_prices = [start_price]
        
        for i in range(1, n_days):
            # Базовий рух ціни з трендом
            daily_return = np.random.normal(trend, volatility)
            
            # Додаємо невелику циклічність
        ...
        # Створюємо DataFrame з усіма необхідними колонками
        df = pd.DataFrame()
        df['date'] = dates
        df['close'] = close_prices
        
        # Генеруємо open, high, low на основі ціни закриття
        ...
        # Корегуємо, щоб high був завжди найвищим, а low - найнижчим
